...
content screens. It also queues Finding evaluation through the real API, waits through the worker,
verifies its pinned Scan/Source evidence and evaluator identity, resets the Site Finding layer,
proves the retained Source refresh remains, rebuilds deterministically from identical evidence, and
reconciles the active Finding summary
