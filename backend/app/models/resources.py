...
if TYPE_CHECKING:
    ...
    from app.models.rendered import RenderedObservation, RenderRun
...
class BackgroundJob(Base):
    ...
    accessibility_run: Mapped["AccessibilityRun | None"] = relationship(back_populates="jobs")
    render_run: Mapped["RenderRun | None"] = relationship(back_populates="jobs")
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            "AND scan_comparison_id IS NULL AND performance_run_id IS NULL "
            "AND accessibility_run_id IS NULL AND render_run_id IS NULL) OR "
            ...
            "AND accessibility_run_id IS NULL AND render_run_id IS NULL "
            ...
            "AND render_run_id IS NOT NULL AND job_type = 'render_run') OR "
            "(scan_id IS NULL AND source_refresh_id IS NULL AND scan_comparison_id IS NULL "
            "AND performance_run_id IS NULL AND accessibility_run_id IS NULL "
            "AND render_run_id IS NULL "
