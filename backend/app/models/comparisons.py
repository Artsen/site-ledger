...
class ScanComparisonPageResult(Base):
    ...
    exact_source_state: Mapped[str] = mapped_column(String(32), index=True)
    exact_source_changed: Mapped[bool] = mapped_column(Boolean, default=False)
    baseline_normalized_source_hash: Mapped[str | None] = mapped_column(String(64))
    target_normalized_source_hash: Mapped[str | None] = mapped_column(String(64))
    normalized_source_state: Mapped[str] = mapped_column(String(32), index=True)
    document_content_state: Mapped[str] = mapped_column(String(32), index=True)
    metadata_state: Mapped[str] = mapped_column(String(32), index=True)
    technical_state: Mapped[str] = mapped_column(String(32), index=True)
    primary_change_class: Mapped[str] = mapped_column(String(32), index=True)
    normalization_only_changed: Mapped[bool] = mapped_column(Boolean, default=False)
    source_difference_categories_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    normalization_details_json: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
