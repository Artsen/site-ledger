...
def test_performance_migration_round_trip(tmp_path: Path, monkeypatch) -> None:
    database_path = tmp_path / "performance-migration.db"
    ...
    try:
        ...
        assert "performance_run_id" in columns
        assert foreign_keys == []

        command.downgrade(config, "202608070020")
        with sqlite3.connect(database_path) as connection:
            tables = {
                row[0]
                for row in connection.execute("SELECT name FROM sqlite_master WHERE type='table'")
            }
        assert "performance_runs" not in tables
