...
import { Activity, Accessibility, Braces, FileStack, Gauge, GitCompareArrows, MonitorUp, Settings } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-md border border-stone-200 bg-white px-4">
          ...
          <EvidencePanel title="Retained render evidence" area="rendered" icon={MonitorUp} coverage={data.render.retained_coverage} clock={data.render.clock} site={site}><p className="mt-2 text-xs text-stone-600">Latest Run targeted {data.render.latest_run.target_count} Pages; retained Site coverage is calculated independently.</p></EvidencePanel>
          ...
          <section className="border-t border-stone-200 py-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Gauge className="h-4 w-4 text-stone-500"/><h3 className="text-sm font-semibold">Performance contexts</h3></div><Link className="text-xs font-medium underline" to={siteAreaHref(site.id, "performance")}>Open</Link></div>{data.performance.contexts.length ? <div className="mt-3 divide-y divide-stone-100 border-y border-stone-100">{data.performance.contexts.map((context) => <div className="py-3" key={`${context.provider}:${context.dimension}:${context.target_kind}:${context.provider_adapter_version}`}><div className="flex justify-between gap-2 text-sm"><span className="font-medium">{formatStatus(context.provider)} / {formatStatus(context.dimension)}</span><span className="text-xs text-stone-500">{context.ready} ready, {context.unavailable} unavailable, {context.failed} failed</span></div><Coverage value={context.coverage} /></div>)}</div> : <p className="mt-2 text-xs text-stone-500">No Performance evidence. Missing measurements do not imply good performance.</p>}</section>
        </section>
