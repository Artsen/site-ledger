...
import type { CollectionCoverage, CollectionPlanRequest, CollectionTargetMode } from "../../types/collectionPlans";
...
function planRequest(coverage: CollectionCoverage, targetMode: CollectionTargetMode = "missing_current"): CollectionPlanRequest {
  const context = coverage.evidence_domain === "performance"
    ...
    : coverage.evidence_domain === "accessibility"
      ...
      : {};
  return { evidence_domain: coverage.evidence_domain, target_mode: targetMode, context };
}
...
function CollectionCoverageActions({ site, values }: { site: Site; values: CollectionCoverage[] }) {
  ...
  const mutation = useMutation({
    mutationFn: async ({ coverage, targetMode }: { coverage: CollectionCoverage; targetMode: CollectionTargetMode }) => {
      const payload = planRequest(coverage, targetMode);
      const preview = await previewCollectionPlan(site.id, payload);
      if (!preview.target_total) throw new Error(targetMode === "refresh_current" ? "No refreshable targets remain for this context." : "No missing current evidence remains for this context.");
      const batchSizes = Array.from({ length: preview.estimated_batch_count }, (_, index) => Math.min(preview.batch_size, preview.target_total - index * preview.batch_size));
      const refresh = targetMode === "refresh_current";
      const confirmed = window.confirm(`${refresh ? "Refresh current" : "Collect missing"} ${formatStatus(preview.evidence_domain)} evidence for ${preview.target_total.toLocaleString()} Page${preview.target_total === 1 ? "" : "s"}?\n\n${preview.covered.toLocaleString()} eligible Pages already have compatible evidence.\n${preview.missing.toLocaleString()} eligible Pages are missing compatible evidence.\n${preview.active_collection.toLocaleString()} eligible Pages are already being collected and will not be duplicated.\n${preview.ineligible.toLocaleString()} active Pages are ineligible.\n\nCurrent identity:\n${preview.context_identity}\n\nPer-Run limit: ${preview.batch_size.toLocaleString()} Pages\nSite Ledger will create ${preview.estimated_batch_count} bounded batch${preview.estimated_batch_count === 1 ? "" : "es"}:\n${batchSizes.map((size, index) => `Batch ${index + 1}: ${size.toLocaleString()}`).join("\n")}\n\n${refresh ? "Existing observations will be retained. This Plan creates new evidence." : "Targets will be frozen when collection starts."}`);
  ...
  return (
    <section className="border-b border-stone-200 py-4">
      <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Current collection contexts</h3><p className="mt-1 text-xs text-stone-500">Fill missing evidence or deliberately collect another compatible observation.</p></div><Link className="text-xs font-medium underline" to={siteAreaHref(site.id, "collection-plans")}>History</Link></div>
      <div className="mt-3 divide-y divide-stone-100 border-y border-stone-100">{values.map((coverage) => <div key={coverage.context_identity} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><span className="text-sm font-medium">{formatStatus(coverage.evidence_domain)} / {collectionLabel(coverage)}</span><p className="mt-0.5 text-xs text-stone-500">{coverage.covered.toLocaleString()} covered, {coverage.in_flight.toLocaleString()} missing and in flight, {coverage.missing.toLocaleString()} missing{coverage.active_collection ? `, ${coverage.active_collection.toLocaleString()} total active collection` : ""}{coverage.ineligible ? `, ${coverage.ineligible.toLocaleString()} ineligible` : ""}</p>{coverage.evidence_domain === "structured_content" ? <p className="mt-1 text-xs text-stone-500">Derived from the latest retained HTML; time-based refresh does not apply.</p> : null}</div><div className="flex gap-2"><Button type="button" variant="ghost" className="min-h-8 px-2 py-1 text-xs" loading={previewMutation.isPending && previewMutation.variables?.context_identity === coverage.context_identity} disabled={coverage.missing === 0 || previewMutation.isPending} onClick={() => previewMutation.mutate(coverage)}>View missing</Button><Button type="button" className="min-h-8 px-2 py-1 text-xs" loading={mutation.isPending && mutation.variables?.coverage.context_identity === coverage.context_identity && mutation.variables.targetMode === "missing_current"} disabled={!coverage.collectable || coverage.missing - coverage.in_flight === 0 || mutation.isPending} title={coverage.non_collectable_reason ?? undefined} onClick={() => mutation.mutate({ coverage, targetMode: "missing_current" })}><Plus className="mr-1 size-3.5" />Collect missing</Button>{coverage.evidence_domain !== "structured_content" ? <Button type="button" variant="secondary" className="min-h-8 px-2 py-1 text-xs" loading={mutation.isPending && mutation.variables?.coverage.context_identity === coverage.context_identity && mutation.variables.targetMode === "refresh_current"} disabled={!coverage.collectable || coverage.eligible - coverage.active_collection === 0 || mutation.isPending} title={coverage.non_collectable_reason ?? undefined} onClick={() => mutation.mutate({ coverage, targetMode: "refresh_current" })}>Refresh current</Button> : null}</div></div>)}</div>
      {visiblePreview ? <div className="mt-3 rounded-md border border-stone-200 bg-stone-50 p-3"><div className="flex items-center justify-between gap-3"><div><h4 className="text-sm font-semibold">Missing preview</h4><p className="text-xs text-stone-500">Showing {visiblePreview.targets.length} of {visiblePreview.target_total.toLocaleString()} authoritative targets. Ineligible Pages are excluded.</p></div><Button variant="ghost" className="min-h-8 px-2 py-1 text-xs" onClick={() => setVisiblePreview(null)}>Close</Button></div><div className="mt-3 max-h-64 overflow-auto"><table className="min-w-full text-left text-xs"><thead className="text-stone-500"><tr><th className="pb-2 pr-3">Page</th><th className="pb-2 pr-3">Identity</th><th className="pb-2">Reason / source</th></tr></thead><tbody>{visiblePreview.targets.map((target) => <tr key={target.web_resource_id} className="border-t border-stone-200 align-top"><td className="max-w-lg break-all py-2 pr-3 font-mono">{target.requested_url}</td><td className="py-2 pr-3 tabular-nums">{target.web_resource_id}</td><td className="py-2">{formatStatus(target.selection_reason)}{target.latest_compatible_observed_at ? <span className="block text-stone-500">Previous compatible evidence {formatDate(target.latest_compatible_observed_at, { timeZone: site.display_timezone, showTimeZone: true })}</span> : null}{target.source_snapshot_id ? <span className="block text-stone-500">Snapshot {target.source_snapshot_id} / Blob {target.content_blob_id}</span> : null}</td></tr>)}</tbody></table></div></div> : null}
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-md border border-stone-200 bg-white px-4">
          ...
          <EvidencePanel title="Accessibility profile coverage" area="accessibility" icon={Accessibility} coverage={data.accessibility.coverage} clock={data.accessibility.clock} site={site}><p className="mt-2 text-xs text-stone-600">Coverage counts current Page/profile audits across desktop and mobile. {data.accessibility.pages_with_violations} Pages have violations; {data.accessibility.failed_pages} Pages have failed current evidence. Missing profiles are not counted as zero violations.</p></EvidencePanel>
