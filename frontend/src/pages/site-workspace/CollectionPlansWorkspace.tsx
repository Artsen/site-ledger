...
function PlanDetail({ site, planId }: { site: Site; planId: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-md border border-stone-200 bg-white p-4"><div className="flex flex-wrap items-center gap-3"><StatusBadge status={plan.status} /><strong className="tabular-nums">{plan.progress.processed_target_count.toLocaleString()} of {plan.target_count.toLocaleString()} Pages processed</strong></div><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div><dt className="text-xs text-stone-500">Target mode</dt><dd>{targetModeLabel(plan.target_mode)}</dd></div><div><dt className="text-xs text-stone-500">At creation</dt><dd>{plan.covered_count_at_creation} covered / {plan.missing_count_at_creation} missing / {plan.active_collection_count_at_creation} already active</dd></div><div><dt className="text-xs text-stone-500">Frozen batches</dt><dd>{plan.batch_count} at up to {plan.batch_size} Pages</dd></div><div><dt className="text-xs text-stone-500">Target reasons</dt><dd>{plan.selection_reason_counts.missing_current ?? 0} missing / {plan.selection_reason_counts.refresh_current ?? 0} refresh</dd></div><div><dt className="text-xs text-stone-500">Created</dt><dd>{formatDate(plan.created_at, { timeZone: site.display_timezone, showTimeZone: true })}</dd></div></dl></section>
