...
import { RenderedDeletionNotice, RenderObservationDeleteAction, RenderRunDeleteAction, RenderSiteDeleteAction, RenderTargetBulkDeleteAction } from "../components/rendered/RenderedEvidenceDeletionActions";
...
export function SiteRenderedPage() {
  ...
  const [deletionResult, setDeletionResult] = useNavigationDeletionResult();
  ...
  return <div className="space-y-5">
    <RenderedDeletionNotice result={deletionResult} />
...
export function RenderRunPage() {
  ...
  return <div className="space-y-5"><RenderedDeletionNotice result={deletionResult} /><header className="flex flex-wrap items-start justify-between gap-3"><div><Link className="text-sm underline" to={`/sites/${site.id}/rendered`}>Render Runs</Link><h1 className="mt-1 text-xl font-semibold">Run {value.id}</h1></div><div className="flex flex-wrap gap-2">{selected.length ? <><Button type="button" loading={rerender.isPending} onClick={() => rerender.mutate()}><RotateCcw className="mr-2 size-4" />Rerender {selected.length}</Button>{TERMINAL.has(value.status) ? <RenderTargetBulkDeleteAction siteId={String(site.id)} runId={runId} targetIds={selected} onDeleted={(result) => { setDeletionResult(result); setSelected([]); }} /> : null}</> : null}{!TERMINAL.has(value.status) ? <Button type="button" variant="danger" loading={cancel.isPending} onClick={() => cancel.mutate()}>Cancel run</Button> : <RenderRunDeleteAction siteId={String(site.id)} runId={value.id} onDeleted={(result) => navigate(`/sites/${site.id}/rendered`, { state: { evidenceDeletion: result } })} />}</div></header>
...
export function RenderedEvidencePage() {
  ...
  return <div className="space-y-4"><header className="flex flex-wrap items-start justify-between gap-3"><div><Link className="text-sm underline" to={`/sites/${site.id}/rendered/runs/${observation.data.render_run_id}`}>Run {observation.data.render_run_id}</Link><h1 className="mt-1 text-xl font-semibold">Rendered observation {observation.data.id}</h1></div><RenderObservationDeleteAction siteId={String(site.id)} observationId={observationId} onDeleted={(result) => navigate(`/sites/${site.id}/rendered/runs/${observation.data.render_run_id}`, { state: { evidenceDeletion: result } })} /></header><RenderedObservationView observation={observation.data} /></div>;
}

function useNavigationDeletionResult() {
  const location = useLocation();
  const navigate = useNavigate();
  const navigationResult = (location.state as { evidenceDeletion?: RenderDeleteResult } | null)?.evidenceDeletion ?? null;
  const [result, setResult] = useState<RenderDeleteResult | null>(navigationResult);
  useEffect(() => {
    if (!navigationResult) return;
    setResult(navigationResult);
    navigate(`${location.pathname}${location.search}`, { replace: true, state: null });
  }, [location.pathname, location.search, navigate, navigationResult]);
