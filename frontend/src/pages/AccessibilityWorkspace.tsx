...
export function SiteAccessibilityPage() {
  ...
  return <div className="space-y-5">
    ...
    {view === "runs" ? <div className="space-y-4"><RunsTable siteId={String(site.id)} runs={runs.data?.items ?? []} onDeleted={setDeletionResult} /><section className="border-t border-stone-200 pt-4"><h2 className="font-semibold">Accessibility evidence lifecycle</h2><p className="mb-3 mt-1 text-sm text-stone-600">Purge all Accessibility evidence for this Site without deleting Pages or Performance evidence.</p><AccessibilitySiteDeleteAction siteId={String(site.id)} onDeleted={setDeletionResult} /></section></div> : null}
...
function RunsTable({ siteId, runs, onDeleted }: { siteId: string; runs: AccessibilityRun[]; onDeleted: (result: EvidenceDeleteResult) => void }) {
  ...
  return <div className="overflow-x-auto rounded-md border border-stone-200 bg-white"><table className="min-w-full text-left text-sm"><thead className="bg-stone-100 text-xs uppercase text-stone-500"><tr><th className="px-3 py-2">Run</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Profiles</th><th className="px-3 py-2">Progress</th><th className="hidden px-3 py-2 sm:table-cell">Evidence retained</th><th className="hidden px-3 py-2 md:table-cell">Created</th><th className="px-3 py-2">Actions</th></tr></thead><tbody>{runs.map((run) => <tr key={run.id} className="border-t border-stone-100"><td className="px-3 py-2"><Link className="font-medium underline" to={`/sites/${siteId}/accessibility/runs/${run.id}`}>Run {run.id}</Link><span className="block text-xs text-stone-500">{run.target_count} Pages / {run.observation_count} collected</span></td><td className="px-3 py-2"><StatusBadge status={run.presentation_status ?? run.status} /></td><td className="px-3 py-2">{run.configuration_json.profiles.map(formatStatus).join(", ")}</td><td className="px-3 py-2 tabular-nums">{run.completed_count} / {run.observation_count}</td><td className="hidden px-3 py-2 sm:table-cell">{run.retained_observation_count} retained, {run.deleted_observation_count} deleted<span className="block text-xs text-stone-500">{run.retained_ready_count} ready, {run.retained_failed_count} failed</span></td><td className="hidden whitespace-nowrap px-3 py-2 md:table-cell">{formatDate(run.created_at)}</td><td className="px-3 py-2">{TERMINAL.has(run.status) ? <AccessibilityRunDeleteAction compact siteId={siteId} runId={run.id} onDeleted={onDeleted} /> : <span className="text-xs text-stone-500" title="Cancel or wait for this run to finish before deleting it.">Active</span>}</td></tr>)}</tbody></table></div>;
}

function ObservationTable({ siteId, observations }: { siteId: string; observations: AccessibilityObservation[] }) {
  return <div className="overflow-x-auto rounded-md border border-stone-200 bg-white"><table className="min-w-full text-left text-sm"><thead className="bg-stone-100 text-xs uppercase text-stone-500"><tr><th className="px-3 py-2">Page</th><th className="px-3 py-2">Profile / outcome</th><th className="px-3 py-2">Violations</th><th className="px-3 py-2">Needs Review</th><th className="hidden px-3 py-2 md:table-cell">Observed</th><th className="px-3 py-2">Result</th></tr></thead><tbody>{observations.map((item) => <tr key={item.id} className="border-t border-stone-100 align-top"><td className="max-w-sm px-3 py-2"><Link className="block truncate font-mono text-xs underline" to={`/sites/${siteId}/pages/${item.web_resource_id}?tab=accessibility`}>{item.page_url ?? item.requested_url}</Link></td><td className="px-3 py-2"><span className="block font-medium">{formatStatus(item.profile)}</span><StatusBadge status={item.outcome} />{item.error_message ? <span className="mt-1 block max-w-sm text-xs text-red-700">{item.error_message}</span> : null}</td><td className="px-3 py-2 tabular-nums">{item.outcome === "ready" ? `${item.violation_rule_count} rules / ${item.violation_node_count} elements` : "Unavailable"}</td><td className="px-3 py-2 tabular-nums">{item.outcome === "ready" ? `${item.incomplete_rule_count} rules` : "Unavailable"}</td><td className="hidden whitespace-nowrap px-3 py-2 md:table-cell">{formatDate(item.observed_at)}</td><td className="px-3 py-2">{item.payload_sha256 ? <Link className="underline" to={`/sites/${siteId}/accessibility/observations/${item.id}`}>Inspect</Link> : "None"}</td></tr>)}</tbody></table></div>;
}
...
export function AccessibilityRunPage() {
  ...
  return <div className="space-y-5"><EvidenceDeletionNotice result={deletionResult} /><header className="flex flex-wrap items-start justify-between gap-3"><div><Link className="text-sm underline" to={`/sites/${site.id}/accessibility?view=runs`}>Accessibility runs</Link><h1 className="mt-1 text-xl font-semibold">Run {value.id}</h1></div>{!TERMINAL.has(value.status) ? <Button type="button" variant="danger" loading={cancel.isPending} onClick={() => cancel.mutate()}>Cancel run</Button> : <AccessibilityRunDeleteAction siteId={String(site.id)} runId={value.id} onDeleted={(result) => navigate(`/sites/${site.id}/accessibility?view=runs`, { state: { evidenceDeletion: result } })} />}</header><section className="rounded-md border border-stone-200 bg-white p-4"><DefinitionList items={[{ label: "Status", value: <StatusBadge status={value.presentation_status ?? value.status} /> }, { label: "Collection progress", value: `${value.completed_count} of ${value.observation_count} browser audits` }, { label: "Evidence retained", value: `${value.retained_observation_count} retained / ${value.deleted_observation_count} deleted` }, { label: "Retained outcomes", value: `${value.retained_ready_count} ready, ${value.retained_failed_count} failed` }, { label: "Profiles", value: value.configuration_json.profiles.map(formatStatus).join(", ") }, { label: "Effective ruleset", value: `${value.ruleset_profile} (${value.ruleset_rule_count} rules)` }, { label: "Ruleset SHA-256", value: value.ruleset_sha256, copyValue: value.ruleset_sha256 }, { label: "axe-core", value: value.axe_core_version }, { label: "Created", value: formatDate(value.created_at) }, { label: "Finished", value: value.finished_at ? formatDate(value.finished_at) : "In progress" }]} />{value.error_summary ? <p className="mt-4 text-sm text-red-700">{value.error_summary}</p> : null}</section>{value.observations.items.length ? <ObservationTable siteId={String(site.id)} observations={value.observations.items} /> : <EmptyState title="No retained observations" message="No retained observations remain for this run." />}</div>;
}
...
function CollectAccessibilityPanel({ siteId, capabilities, onClose, initialResourceId }: { siteId: string; capabilities: AccessibilityCapabilities; onClose: () => void; initialResourceId?: number }) {
  ...
  return <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="collect-accessibility-title" className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-3 sm:p-8"><div className="mx-auto max-w-3xl rounded-md bg-white p-4 shadow-xl sm:p-6"><header className="flex items-start justify-between gap-3"><div><h2 id="collect-accessibility-title" className="text-lg font-semibold">Run Accessibility Audit</h2><p className="text-sm text-stone-600">Recommended batch: {capabilities.default_page_limit} Pages. Hard limit: {pageLimit}. Browser audits run serially.</p></div><button type="button" aria-label="Close Accessibility audit" onClick={onClose} className="rounded p-2 hover:bg-stone-100"><X size={20} /></button></header>{!initialResourceId ? <div className="mt-5"><CollectionPageSelector siteId={siteId} selected={selected} hardLimit={pageLimit} label="Accessibility" onChange={setSelected} /></div> : null}<fieldset className="mt-5 border-t border-stone-200 pt-5"><legend className="font-medium">Audit profiles</legend><Check label="Desktop (1440 × 900)" checked={desktop} onChange={setDesktop} /><Check label="Mobile (390 × 844)" checked={mobile} onChange={setMobile} /></fieldset><p className="mt-4 rounded-md bg-stone-100 p-3 text-sm"><strong>{selected.length}</strong> Pages × <strong>{profiles.length}</strong> profiles = <strong>{auditCount}</strong> browser audits.</p>{selected.length > capabilities.default_page_limit ? <p className="mt-3 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">This exceeds the recommended {capabilities.default_page_limit}-Page batch and may take substantially longer.</p> : null}{auditCount > capabilities.max_audit_count ? <p className="mt-3 rounded-md border border-red-300 bg-red-50 p-3 text-sm text-red-900">Reduce the selection or profiles to stay within the configured limit of {capabilities.max_audit_count} browser audits.</p> : null}<p className="mt-3 text-xs text-stone-600">Automated detection cannot evaluate every WCAG 2.2 A/AA requirement. Incomplete results are retained as Needs Review evidence.</p><div className="mt-5 flex flex-col gap-3 border-t border-stone-200 pt-4 sm:flex-row sm:items-center sm:justify-end"><Button type="button" onClick={onClose}>Cancel</Button><Button type="button" variant="primary" loading={create.isPending} disabled={!selected.length || !profiles.length || auditCount > capabilities.max_audit_count} onClick={() => create.mutate()}>Start {auditCount} audits</Button></div>{create.error ? <div className="mt-3"><ErrorBanner error={create.error} title="Could not start Accessibility audit" /></div> : null}</div></section>;
}
