...
export function LegacySiteDetailPage() {
  ...
  return (
    <PageFrame>
      ...
      {tab === "overview" ? <SiteOverviewSection site={site.data} /> : null}
      {tab === "scans" ? <SiteRecentScansSection site={site.data} /> : null}
      {tab === "comparisons" ? <SiteComparisonsPanel site={site.data} /> : null}
      {tab === "pages" ? <SitePagesSection site={site.data} /> : null}
      ...
      {tab === "categories" ? <SiteCategoriesSection site={site.data} /> : null}
      {tab === "sources" ? <SiteSourcesSection site={site.data} /> : null}
      {tab === "inventory" ? <SiteInventorySection site={site.data} /> : null}
      {tab === "notes" ? (
        <NotesPanel
...
export function SitePagesSection({ site }: { site: Site }) {
  const [searchParams, setSearchParams] = useSearchParams();
...
export function SiteOverviewSection({ site }: { site: Site }) {
  return (
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <SiteRecentScansSection site={site} compact />
    </div>
  );
}

export function SiteRecentScansSection({
  site,
  compact = false,
}: {
  site: Site;
...
export function SiteSourcesSection({ site, mode = "sources" }: { site: Site; mode?: "sources" | "ai-documents" }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {mode === "ai-documents" ? <section className="rounded-md border border-stone-200 bg-white p-4 shadow-sm">
      ...
      {mode === "sources" ? <section className="rounded-md border border-stone-200 bg-white p-4 shadow-sm">
      ...
      <section className="rounded-md border border-stone-200 bg-white shadow-sm">
        ...
        {displayedSources.length ? (
          ...
          <EmptyState
          title={mode === "ai-documents" ? "No AI Document Sources" : "No sources"}
          message={mode === "ai-documents" ? "Discover or add an AI Document Source to begin." : "Add a sitemap, discover from robots.txt, or paste manual URLs."}
          />
        ) : null}
      </section>
    </div>
              ...
              <tr key={source.id} className="border-t border-stone-100">
                ...
                <td className="px-3 py-2">
                  <div className="flex gap-2">
                    {source.source_type === "ai_document" ? <Link className="underline" to={`/sites/${siteId}/ai-documents/${source.id}`}>Open Source</Link> : null}
                    <button
                      type="button"
