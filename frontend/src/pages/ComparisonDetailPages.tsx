...
export function PageComparisonDetailPage() {
  ...
  const diff = useQuery({ queryKey: ["comparison-source-diff", siteId, comparisonId, resourceId], queryFn: () => getComparisonPageSourceDiff(siteId, comparisonId, resourceId), enabled: showDiff, ...immutableComparisonQueryOptions });
  ...
  return <Frame>
    ...
    <section className="mt-6 border-y border-stone-200 py-5"><h2 className="font-semibold">Tracked differences</h2><div className="mt-3 grid grid-cols-2 gap-3 text-sm md:grid-cols-4">{[["Content", item.content_state], ["Head metadata", item.head_state], ["HTTP status", item.http_status_changed], ["Final URL", item.final_url_changed], ["Redirect", item.redirect_state_changed], ["Canonical", item.canonical_changed], ["Robots", item.robots_changed], ["Language", item.language_changed], ["Inbound links", item.inbound_links_changed], ["Outbound links", item.outbound_links_changed], ["Rendered summary", item.rendered_state_changed || item.rendered_counts_changed], ["Changed fields", item.changed_field_count]].map(([label, value]) => <div key={String(label)}><span className="block text-stone-500">{label}</span><strong>{typeof value === "boolean" ? (value ? "Changed" : "Same") : formatStatus(String(value))}</strong></div>)}</div></section>
