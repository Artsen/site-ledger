...
function Summary({ label, values }: { label: string; values: Record<string, number> }) {
  return <section className="border-t-2 border-neutral-900 pt-3"><h3 className="font-semibold">{label}</h3><dl className="mt-2 grid gap-1 text-sm">{Object.entries(values).map(([key, value]) => <div key={key} className="flex justify-between gap-3"><dt className="text-stone-600">{formatStatus(key)}</dt><dd className="font-medium tabular-nums">{value.toLocaleString()}</dd></div>)}</dl></section>;
}
...
function PagesTable({ items, siteId, comparisonId, sort, direction, onSort }: { items: ComparisonPage[]; siteId: string; comparisonId: string; sort: string | null; direction: SortDirection | null; onSort: (column: string | null, direction: SortDirection | null) => void }) {
  const headers = [["url", "URL"], ["presence", "Presence"], ["change", "Classification"], ["content", "Content"], ["metadata", "Metadata"], ["technical", "Technical"], ["raw_source", "Raw source"], ["changed_field_count", "Changed fields"]];
  return <Table><thead className="bg-stone-100 text-xs uppercase text-stone-500"><tr>{headers.map(([column, label]) => <SortableTableHeader key={column} column={column} label={label} activeColumn={sort} direction={direction} onChange={onSort} defaultDirection={column === "changed_field_count" ? "desc" : "asc"} />)}<th className="px-3 py-2">Actions</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t border-stone-100 align-top"><td className="max-w-md px-3 py-2"><Link className="break-all font-mono text-xs underline" to={`/sites/${siteId}/comparisons/${comparisonId}/pages/${item.resource_id}`}>{item.normalized_url}</Link></td><td className="px-3 py-2"><StatusBadge status={item.presence_state} label={formatStatus(item.presence_state)} /></td><td className="px-3 py-2"><StatusBadge status={item.primary_change_class} label={formatStatus(item.primary_change_class)} /></td><td className="px-3 py-2">{formatStatus(item.document_content_state)}</td><td className="px-3 py-2">{formatStatus(item.metadata_state)}</td><td className="px-3 py-2">{formatStatus(item.technical_state)}</td><td className="px-3 py-2">{formatStatus(item.exact_source_state)}</td><td className="px-3 py-2 tabular-nums">{item.changed_field_count}</td><td className="px-3 py-2"><Link className="underline" to={`/sites/${siteId}/comparisons/${comparisonId}/pages/${item.resource_id}`}>Open</Link></td></tr>)}</tbody></Table>;
}
