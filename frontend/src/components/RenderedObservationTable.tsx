...
export function RenderedObservationTable({ scanId, renderMode, queryKey, loadObservations, observationHref, selectedTargetIds = [], onSelectedTargetIdsChange }: RenderedObservationTableProps) {
  ...
  return <div className="space-y-4">
    ...
    {rendered.data?.items.length ? <div className="overflow-x-auto rounded-md border border-stone-200 bg-white shadow-sm"><table className="min-w-full text-left text-sm"><thead className="bg-stone-100 text-xs uppercase text-stone-500"><tr>{onSelectedTargetIdsChange ? <th className="w-10 px-3 py-2"><input type="checkbox" aria-label="Select all rendered observations on this page" checked={Boolean(rendered.data.items.length) && rendered.data.items.every((item) => item.render_run_target_id != null && selectedTargetIds.includes(item.render_run_target_id))} onChange={(event) => onSelectedTargetIdsChange(event.target.checked ? [...new Set([...selectedTargetIds, ...rendered.data.items.flatMap((item) => item.render_run_target_id == null ? [] : [item.render_run_target_id])])] : selectedTargetIds.filter((id) => !rendered.data?.items.some((item) => item.render_run_target_id === id)))} /></th> : null}{[["page_url", "Page"], ["capture_state", "Capture"], ["navigation_status", "Navigation"], ["duration", "Duration"], ["warning_count", "Warnings"], ["browser_evidence", "Browser evidence"], ["capture_time", "Captured"]].map(([column, label]) => <SortableTableHeader key={column} column={column} label={label} activeColumn={searchParams.get("sort")} direction={searchParams.get("direction") as SortDirection | null} onChange={(column, direction) => setRenderedSort(setSearchParams, column, direction)} defaultDirection={column === "capture_time" ? "desc" : "asc"} />)}</tr></thead><tbody>{rendered.data.items.map((item) => <tr key={item.id} className="border-t border-stone-100 align-top hover:bg-stone-50">
      {onSelectedTargetIdsChange ? <td className="px-3 py-2"><input type="checkbox" aria-label={`Select ${item.static_final_url ?? item.page_title ?? `observation ${item.id}`}`} disabled={item.render_run_target_id == null} checked={item.render_run_target_id != null && selectedTargetIds.includes(item.render_run_target_id)} onChange={(event) => item.render_run_target_id != null && onSelectedTargetIdsChange(event.target.checked ? [...selectedTargetIds, item.render_run_target_id] : selectedTargetIds.filter((id) => id !== item.render_run_target_id))} /></td> : null}
